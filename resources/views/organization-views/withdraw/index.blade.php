                ...
                <div class="flex-between __wrap-gap-10 align-items-center">
                    <div>
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2" href="{{route('organization.withdraw.download', ['withdrawal_method'=>$method,'search'=>$search, 'request_status'=>$requestStatus])}}">
                            ...
                            <td>
                                @if($withdraw_request->user)
                                    <span class="d-block font-size-sm text-body">
                                        <a href="{{route('organization.customer.view',[$withdraw_request->user->id])}}">
                            ...
                            <td>
                                @if( $withdraw_request->request_status == 'pending' )
                                    <div class="d-flex gap-2">
                                        <a href="{{ route('organization.withdraw.status_update', ['request_id'=>$withdraw_request->id, 'request_status'=>'approve']) }}" class="btn btn-primary btn-sm"> {{translate('Approve')}}</a>
                                        <a href="{{ route('organization.withdraw.status_update', ['request_id'=>$withdraw_request->id, 'request_status'=>'deny']) }}" class="btn btn-warning btn-sm"> {{translate('Deny')}}</a>
                                    </div>
...
@push('script_2')
    <script>
        ...
        $("#withdrawal_method").on('change', function (event) {
            location.href = "{{route('organization.withdraw.requests')}}" + '?request_status=all' + '&withdrawal_method=' + $(this).val();
        })
    </script>
@endpush
