...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="card card-body mb-3">
            <form action="{{route('organization.transfer.store')}}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-4">
                    ...
                    @foreach($transfers as $key=>$transfer)
                        <tr>
                            ...
                            <td>
                                ...
                                @if(isset($orgInfo))
                                    <a href="{{route('organization.customer.view',[$orgInfo['id']])}}">{{ $orgInfo->org_name}}</a>
                                @else
                            ...
                            <td>
                                ...
                                @if(isset($userInfo))
                                    <a href="{{route('organization.customer.view',[$userInfo['id']])}}">{{ $userInfo->f_name . ' ' . $userInfo->l_name }}</a>
                                @else
                                    <span class="text-muted badge badge-danger text-dark">{{ translate('User unavailable') }}</span>
                                @endif
                            </td>
...
@push('script_2')
    <script>
        ...
        $("#receiver").select2({
            ajax: {
                url: '{{route('organization.transfer.get_user')}}',
                type: "get",
